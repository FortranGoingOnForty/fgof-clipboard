program test_clipboard_failures
  ...
  if (result_value%success) error stop "copy failures should not report success: backend=" // result_value%backend // " text=" // result_value%text
  ...
  if (.not. result_value%success) error stop "clipboard writes should recover after copy failure: backend=" // result_value%backend // " error=" // result_value%error_message
